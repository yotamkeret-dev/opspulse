...
import {
  ...
  getWeekTag,
} from '@/lib/procurement/import-executor';
import {
...
async function updateProcurementInDB(
  ...
  if (oldRecord && changedBy) {
    await insertProcurementHistoryRows(supabase, id, oldRecord, patch, changedBy);
  }
}
      ...
      <aside className="sidebar">
        ...
        <a
          ...
          style={{
            ...
            boxShadow:'0 2px 8px rgba(29,46,107,.35)',
          }}
        >
          ← Ops Hub
        </a>
...
function ProcurementPage({ timeFilter, activeTeamMembers, authUserEmail, authUserId, onRecordAdded, onRecordDeleted, supportLogs, onUpdateLog }: {
  ...
  const handleBulkImport = async (mapped: MappedRecord<ProcurementRecord>[]) => {
    ...
    const { upserted, inserted } = await executeProcurementImport(mapped, {
      supabase:            createClient(),
      companyId:           procCompanyId!,
      callerEmail:         authUserEmail ?? '',
      callerId:            authUserId ?? null,
      defaultEmployeeName: defaultEmployee?.name ?? 'Operations Team',
      logActivity: authUserId
        ? (entry) => insertLogToDB(entry, authUserId, authUserEmail ?? '', procCompanyId!)
        : undefined,
    });

    // Update local state from executor results, mirroring the original per-record setRecords calls.
    setRecords(prev => {
      let next = prev;
      for (const r of upserted) {
        const exists = next.some(x => x.id === r.id);
        next = exists ? next.map(x => x.id === r.id ? r : x) : [...next, r];
      }
